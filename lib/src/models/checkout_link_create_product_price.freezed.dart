// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'checkout_link_create_product_price.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#adding-getters-and-methods-to-our-models');

CheckoutLinkCreateProductPrice _$CheckoutLinkCreateProductPriceFromJson(
    Map<String, dynamic> json) {
  return _CheckoutLinkCreateProductPrice.fromJson(json);
}

/// @nodoc
mixin _$CheckoutLinkCreateProductPrice {
  Map<String, dynamic>? get metadata => throw _privateConstructorUsedError;
  dynamic? get trial_interval => throw _privateConstructorUsedError;
  dynamic? get trial_interval_count => throw _privateConstructorUsedError;
  String get payment_processor => throw _privateConstructorUsedError;
  String? get label => throw _privateConstructorUsedError;
  bool? get allow_discount_codes => throw _privateConstructorUsedError;
  bool? get require_billing_address => throw _privateConstructorUsedError;
  String? get discount_id => throw _privateConstructorUsedError;
  String? get success_url => throw _privateConstructorUsedError;
  String? get return_url => throw _privateConstructorUsedError;
  String get product_price_id => throw _privateConstructorUsedError;

  /// Serializes this CheckoutLinkCreateProductPrice to a JSON map.
  Map<String, dynamic> toJson() => throw _privateConstructorUsedError;

  /// Create a copy of CheckoutLinkCreateProductPrice
  /// with the given fields replaced by the non-null parameter values.
  @JsonKey(includeFromJson: false, includeToJson: false)
  $CheckoutLinkCreateProductPriceCopyWith<CheckoutLinkCreateProductPrice>
      get copyWith => throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $CheckoutLinkCreateProductPriceCopyWith<$Res> {
  factory $CheckoutLinkCreateProductPriceCopyWith(
          CheckoutLinkCreateProductPrice value,
          $Res Function(CheckoutLinkCreateProductPrice) then) =
      _$CheckoutLinkCreateProductPriceCopyWithImpl<$Res,
          CheckoutLinkCreateProductPrice>;
  @useResult
  $Res call(
      {Map<String, dynamic>? metadata,
      dynamic? trial_interval,
      dynamic? trial_interval_count,
      String payment_processor,
      String? label,
      bool? allow_discount_codes,
      bool? require_billing_address,
      String? discount_id,
      String? success_url,
      String? return_url,
      String product_price_id});
}

/// @nodoc
class _$CheckoutLinkCreateProductPriceCopyWithImpl<$Res,
        $Val extends CheckoutLinkCreateProductPrice>
    implements $CheckoutLinkCreateProductPriceCopyWith<$Res> {
  _$CheckoutLinkCreateProductPriceCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  /// Create a copy of CheckoutLinkCreateProductPrice
  /// with the given fields replaced by the non-null parameter values.
  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? metadata = freezed,
    Object? trial_interval = freezed,
    Object? trial_interval_count = freezed,
    Object? payment_processor = null,
    Object? label = freezed,
    Object? allow_discount_codes = freezed,
    Object? require_billing_address = freezed,
    Object? discount_id = freezed,
    Object? success_url = freezed,
    Object? return_url = freezed,
    Object? product_price_id = null,
  }) {
    return _then(_value.copyWith(
      metadata: freezed == metadata
          ? _value.metadata
          : metadata // ignore: cast_nullable_to_non_nullable
              as Map<String, dynamic>?,
      trial_interval: freezed == trial_interval
          ? _value.trial_interval
          : trial_interval // ignore: cast_nullable_to_non_nullable
              as dynamic?,
      trial_interval_count: freezed == trial_interval_count
          ? _value.trial_interval_count
          : trial_interval_count // ignore: cast_nullable_to_non_nullable
              as dynamic?,
      payment_processor: null == payment_processor
          ? _value.payment_processor
          : payment_processor // ignore: cast_nullable_to_non_nullable
              as String,
      label: freezed == label
          ? _value.label
          : label // ignore: cast_nullable_to_non_nullable
              as String?,
      allow_discount_codes: freezed == allow_discount_codes
          ? _value.allow_discount_codes
          : allow_discount_codes // ignore: cast_nullable_to_non_nullable
              as bool?,
      require_billing_address: freezed == require_billing_address
          ? _value.require_billing_address
          : require_billing_address // ignore: cast_nullable_to_non_nullable
              as bool?,
      discount_id: freezed == discount_id
          ? _value.discount_id
          : discount_id // ignore: cast_nullable_to_non_nullable
              as String?,
      success_url: freezed == success_url
          ? _value.success_url
          : success_url // ignore: cast_nullable_to_non_nullable
              as String?,
      return_url: freezed == return_url
          ? _value.return_url
          : return_url // ignore: cast_nullable_to_non_nullable
              as String?,
      product_price_id: null == product_price_id
          ? _value.product_price_id
          : product_price_id // ignore: cast_nullable_to_non_nullable
              as String,
    ) as $Val);
  }
}

/// @nodoc
abstract class _$$CheckoutLinkCreateProductPriceImplCopyWith<$Res>
    implements $CheckoutLinkCreateProductPriceCopyWith<$Res> {
  factory _$$CheckoutLinkCreateProductPriceImplCopyWith(
          _$CheckoutLinkCreateProductPriceImpl value,
          $Res Function(_$CheckoutLinkCreateProductPriceImpl) then) =
      __$$CheckoutLinkCreateProductPriceImplCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call(
      {Map<String, dynamic>? metadata,
      dynamic? trial_interval,
      dynamic? trial_interval_count,
      String payment_processor,
      String? label,
      bool? allow_discount_codes,
      bool? require_billing_address,
      String? discount_id,
      String? success_url,
      String? return_url,
      String product_price_id});
}

/// @nodoc
class __$$CheckoutLinkCreateProductPriceImplCopyWithImpl<$Res>
    extends _$CheckoutLinkCreateProductPriceCopyWithImpl<$Res,
        _$CheckoutLinkCreateProductPriceImpl>
    implements _$$CheckoutLinkCreateProductPriceImplCopyWith<$Res> {
  __$$CheckoutLinkCreateProductPriceImplCopyWithImpl(
      _$CheckoutLinkCreateProductPriceImpl _value,
      $Res Function(_$CheckoutLinkCreateProductPriceImpl) _then)
      : super(_value, _then);

  /// Create a copy of CheckoutLinkCreateProductPrice
  /// with the given fields replaced by the non-null parameter values.
  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? metadata = freezed,
    Object? trial_interval = freezed,
    Object? trial_interval_count = freezed,
    Object? payment_processor = null,
    Object? label = freezed,
    Object? allow_discount_codes = freezed,
    Object? require_billing_address = freezed,
    Object? discount_id = freezed,
    Object? success_url = freezed,
    Object? return_url = freezed,
    Object? product_price_id = null,
  }) {
    return _then(_$CheckoutLinkCreateProductPriceImpl(
      metadata: freezed == metadata
          ? _value._metadata
          : metadata // ignore: cast_nullable_to_non_nullable
              as Map<String, dynamic>?,
      trial_interval: freezed == trial_interval
          ? _value.trial_interval
          : trial_interval // ignore: cast_nullable_to_non_nullable
              as dynamic?,
      trial_interval_count: freezed == trial_interval_count
          ? _value.trial_interval_count
          : trial_interval_count // ignore: cast_nullable_to_non_nullable
              as dynamic?,
      payment_processor: null == payment_processor
          ? _value.payment_processor
          : payment_processor // ignore: cast_nullable_to_non_nullable
              as String,
      label: freezed == label
          ? _value.label
          : label // ignore: cast_nullable_to_non_nullable
              as String?,
      allow_discount_codes: freezed == allow_discount_codes
          ? _value.allow_discount_codes
          : allow_discount_codes // ignore: cast_nullable_to_non_nullable
              as bool?,
      require_billing_address: freezed == require_billing_address
          ? _value.require_billing_address
          : require_billing_address // ignore: cast_nullable_to_non_nullable
              as bool?,
      discount_id: freezed == discount_id
          ? _value.discount_id
          : discount_id // ignore: cast_nullable_to_non_nullable
              as String?,
      success_url: freezed == success_url
          ? _value.success_url
          : success_url // ignore: cast_nullable_to_non_nullable
              as String?,
      return_url: freezed == return_url
          ? _value.return_url
          : return_url // ignore: cast_nullable_to_non_nullable
              as String?,
      product_price_id: null == product_price_id
          ? _value.product_price_id
          : product_price_id // ignore: cast_nullable_to_non_nullable
              as String,
    ));
  }
}

/// @nodoc
@JsonSerializable()
class _$CheckoutLinkCreateProductPriceImpl
    implements _CheckoutLinkCreateProductPrice {
  const _$CheckoutLinkCreateProductPriceImpl(
      {final Map<String, dynamic>? metadata,
      this.trial_interval,
      this.trial_interval_count,
      required this.payment_processor,
      this.label,
      this.allow_discount_codes,
      this.require_billing_address,
      this.discount_id,
      this.success_url,
      this.return_url,
      required this.product_price_id})
      : _metadata = metadata;

  factory _$CheckoutLinkCreateProductPriceImpl.fromJson(
          Map<String, dynamic> json) =>
      _$$CheckoutLinkCreateProductPriceImplFromJson(json);

  final Map<String, dynamic>? _metadata;
  @override
  Map<String, dynamic>? get metadata {
    final value = _metadata;
    if (value == null) return null;
    if (_metadata is EqualUnmodifiableMapView) return _metadata;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableMapView(value);
  }

  @override
  final dynamic? trial_interval;
  @override
  final dynamic? trial_interval_count;
  @override
  final String payment_processor;
  @override
  final String? label;
  @override
  final bool? allow_discount_codes;
  @override
  final bool? require_billing_address;
  @override
  final String? discount_id;
  @override
  final String? success_url;
  @override
  final String? return_url;
  @override
  final String product_price_id;

  @override
  String toString() {
    return 'CheckoutLinkCreateProductPrice(metadata: $metadata, trial_interval: $trial_interval, trial_interval_count: $trial_interval_count, payment_processor: $payment_processor, label: $label, allow_discount_codes: $allow_discount_codes, require_billing_address: $require_billing_address, discount_id: $discount_id, success_url: $success_url, return_url: $return_url, product_price_id: $product_price_id)';
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$CheckoutLinkCreateProductPriceImpl &&
            const DeepCollectionEquality().equals(other._metadata, _metadata) &&
            const DeepCollectionEquality()
                .equals(other.trial_interval, trial_interval) &&
            const DeepCollectionEquality()
                .equals(other.trial_interval_count, trial_interval_count) &&
            (identical(other.payment_processor, payment_processor) ||
                other.payment_processor == payment_processor) &&
            (identical(other.label, label) || other.label == label) &&
            (identical(other.allow_discount_codes, allow_discount_codes) ||
                other.allow_discount_codes == allow_discount_codes) &&
            (identical(
                    other.require_billing_address, require_billing_address) ||
                other.require_billing_address == require_billing_address) &&
            (identical(other.discount_id, discount_id) ||
                other.discount_id == discount_id) &&
            (identical(other.success_url, success_url) ||
                other.success_url == success_url) &&
            (identical(other.return_url, return_url) ||
                other.return_url == return_url) &&
            (identical(other.product_price_id, product_price_id) ||
                other.product_price_id == product_price_id));
  }

  @JsonKey(includeFromJson: false, includeToJson: false)
  @override
  int get hashCode => Object.hash(
      runtimeType,
      const DeepCollectionEquality().hash(_metadata),
      const DeepCollectionEquality().hash(trial_interval),
      const DeepCollectionEquality().hash(trial_interval_count),
      payment_processor,
      label,
      allow_discount_codes,
      require_billing_address,
      discount_id,
      success_url,
      return_url,
      product_price_id);

  /// Create a copy of CheckoutLinkCreateProductPrice
  /// with the given fields replaced by the non-null parameter values.
  @JsonKey(includeFromJson: false, includeToJson: false)
  @override
  @pragma('vm:prefer-inline')
  _$$CheckoutLinkCreateProductPriceImplCopyWith<
          _$CheckoutLinkCreateProductPriceImpl>
      get copyWith => __$$CheckoutLinkCreateProductPriceImplCopyWithImpl<
          _$CheckoutLinkCreateProductPriceImpl>(this, _$identity);

  @override
  Map<String, dynamic> toJson() {
    return _$$CheckoutLinkCreateProductPriceImplToJson(
      this,
    );
  }
}

abstract class _CheckoutLinkCreateProductPrice
    implements CheckoutLinkCreateProductPrice {
  const factory _CheckoutLinkCreateProductPrice(
          {final Map<String, dynamic>? metadata,
          final dynamic? trial_interval,
          final dynamic? trial_interval_count,
          required final String payment_processor,
          final String? label,
          final bool? allow_discount_codes,
          final bool? require_billing_address,
          final String? discount_id,
          final String? success_url,
          final String? return_url,
          required final String product_price_id}) =
      _$CheckoutLinkCreateProductPriceImpl;

  factory _CheckoutLinkCreateProductPrice.fromJson(Map<String, dynamic> json) =
      _$CheckoutLinkCreateProductPriceImpl.fromJson;

  @override
  Map<String, dynamic>? get metadata;
  @override
  dynamic? get trial_interval;
  @override
  dynamic? get trial_interval_count;
  @override
  String get payment_processor;
  @override
  String? get label;
  @override
  bool? get allow_discount_codes;
  @override
  bool? get require_billing_address;
  @override
  String? get discount_id;
  @override
  String? get success_url;
  @override
  String? get return_url;
  @override
  String get product_price_id;

  /// Create a copy of CheckoutLinkCreateProductPrice
  /// with the given fields replaced by the non-null parameter values.
  @override
  @JsonKey(includeFromJson: false, includeToJson: false)
  _$$CheckoutLinkCreateProductPriceImplCopyWith<
          _$CheckoutLinkCreateProductPriceImpl>
      get copyWith => throw _privateConstructorUsedError;
}
